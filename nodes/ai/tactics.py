...
class Tactics():
	...
	def get_behind_ball(self, robot_me, ball):
		...
		point = util.get_point_behind_ball(ball, soccerobjects.OPP_GOAL)
	...
	def run_to_goal(self, robot_me, ball):
		...
		while dist < constants.POSSESSION_DIST and angle_diff < constants.MAX_ANGLE_DIFF:
			self.skills.move_to_goal(util.get_angle_from_points(robot_me.location, soccerobjects.OPP_GOAL), soccerobjects.OPP_GOAL)
			dist = robot_me.location.distance_from(ball)
			angle_diff = util.get_angle_diff_from_points(robot_me.location, ball, soccerobjects.OPP_GOAL)
			# print dist, angle_diff
