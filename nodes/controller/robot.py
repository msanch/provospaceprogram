...
from ai.soccerobjects import GameState
...
class Robot(object):
    ...
    def __init__(self, psp_number, is_team_home=True, velocity=(0, 0), angular_velocity=0,
        ...
        if not debug:
            ...
            rospy.Subscriber('/game_state', GameStateMsg, self.game_state.update)
...
# FIXME Get corrections for rotating on xy plane
    ...
    i = 0
    def run(self):
        # side = 1 if self.is_team_home ^ self.game_state.second_half else -1
        delta_x = (self.current_position[0] - self.desired_position[0]) # * side
        delta_y = (self.current_position[1] - self.desired_position[1]) # * side
...
def main():
    ...
    # is_team_home = sys.argv[2] == "home"  # rospy.get_param('~is_team_home')
    robot = Robot(psp_number)  # , is_team_home)
